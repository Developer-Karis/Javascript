...
// Créer une liste des élèves admis à Molengeek en fonction de ses présences 
// et de la note qu'il a obtenu pour le site du chef
...
// ## 3
// Vous avez un aquarium avec des crevettes, des poissons, des crabes et des escargots, 2 de chaque. 
// Comme c'est le bordel, vous souhaitez séparer toutes les espèces dans 4 bacs 
...
// let aquarium = ["Crevette géante tigrée", "Crevette d'Amano", "Espadon", 
// "Poisson rouge", "Crabe de cocotier", "Maja squinado", "Helix aspersa", "Bulime tronqué"];
...
//     const aliment = 1;
//     const boisson = 2;
//     age = prompt("Vous pouvez vivre jusqu'à quel âge ? ");
//     let resultAliment = aliment * 365 * age;
//     let resultBoisson = boisson * 365 * age;
//     console.log("Tu auras besoin de " + resultAliment
//         + " aliments jusqu'à ce que tu arrives a l'age de " + age + " ans. ");
//     console.log("Tu auras besoin de " + resultBoisson
//         + " boissons jusqu'à ce que tu arrives a l'age de " + age + " ans. ");
// }
// quantity();
...
// ## 7
// //Créer une fonction [calcul] qui va demander à l'utilisateur la réponse au calcul ((10*50)/2) 
// et vérifie si il a bien entré la bonne réponse.
...
// // L'utilisateur à 4 chances pour répondre, et 10sec d'intervalle avant de pouvoir réessayer. 
// Stocks les réponses fausses dans un tableau , et la vraie dans un autre.

// ## 8
// 1.
// crée une fonction avec lequel l'utilisateur serait en mesure de choisir trois éléments 
// pour son menu en te basant sur les tableau suivant:
...
// 2.
// la fonction devras afficher via une alerte le prix total du menu. 
// Pour les prix tu te base sur les références ci dessous:
...
// 2. si la longueur est semblable à la largeur, il doit alors renvoyer que c'est un carré, 
// il doit donc entrer une nouvelle valeur afin qu'il devienne un rectangle tout en affichant sa surface en "cm"
...
// // crée trois autres tableaux avec comme nom "en dessous de la moyenne"(<1600 euro), 
// "dans la moyenne"(entre 1600 et 1800 euro) et "au dessus de la moyenne"(>1800)
...
// Vous avez besoin de 3 tableaux différents(prénom des élèves, côte des élèves, 
// concatener les prénoms et les côtes des élèves).
// Il peut rajouter un élève par la suite et il peut aussi arrêter le programme 
// à la fin en lui posant la question(Oui ou Non).

/*----- Déjà fais ----- */

// ## 14
// Créer un tableau avec plusieurs humains et gobelins(8 element max)
// Créer une fonction pistolet qui permet de tuer et qui tramsforme l'element "humain" 
// en "humain mort" et "Gobelins" en "Gobelins mort".
// créer une fonction vue qui permet de voir chaque element du tableau 
// mais que vous avez 1 chance sur 2 de "ne pas bien voir" l'element (car vous êtes borgne) 
// et que ce lui ci devient un "element invisible".
// dans la fonction vue faite appelle à Pistolet pour commencer la purge.
// Une fois la purge faite allez chercher vos cartouches en comptant(faites un compteur) 
// le nbr de gobelins mort et le nbr d'humains mort. 
// Si vous avez tué tout les humains mettez une alert assasin par contre 
// si ce sont tout les gobelins qui sont mort mettez une alert Heros par contrre 
// si vous tuez tout le monde alors une alert vous êtes un psycopathe devrait faire son appaition
// Bonus: faire un tableau qui se mélange(Un randomizer).

// ## 15
// - Cree une fonction pour une maison medicale(peux prendre max 10personne) 
// ou les utilisateur rentre leurs données(nom, prenom, age, raison d'admission et genre). 
// - A la fin toute les information / tableau(homme, femme, tout le monde ensemble) doivent s'afficher dans le console.log

// ## 16
// 1. créer un progamme qui va mettre deux chiffres introduit par l'utilisateur cote à cote et dire 
// si le chiffre obtenu est divisble par 2.
...
// 4. si le chiffre est divisble par 2, le mettre dans le tableau[divi]. 
// s'il n'est pas divisble le mettre dans le tableau[pasDivi]